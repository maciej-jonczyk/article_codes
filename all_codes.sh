...
# 2. Adapter and quality trimming. bbduk from bbmap suite was used. Packake can be downloaded from https://sourceforge.net/projects/bbmap/.
...
# 3. Quality control, as in step 1.

# 4. Mapping to B73 maize genome version 4. bbmap script from the bbmap suite.
...
# 5. Quality control of bam files. bamqc program was used, avaliable from https://github.com/s-andrews/BamQC
# For analysis of all files in the current directory *.bam expression can be used. Resulting files have added ".html" at the end.
bamqc *.bam

# 6. Filtering, only reads with MAPQ score at least 10 were retained
for i in {4..39};
do samtools view -@24 -bq10 bbd4${i}.bam -o bb${i}q10_4.bam;
done

# 7. Removal of reads mapping to organellar DNA
